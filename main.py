...
async def album_event_handler(event):
    ...
    if moderation_active:
        ...
        destination_channel_title = None
        if destination_channel_id is not None:
        ...
        if destination_channel_id is not None and destination_channel_title is not None:
            await bot.send_message(technical_channel_id, f"Выберите действие ({destination_channel_title} - ID {destination_channel_id}):", reply_markup=moderation_keyboard)
        else:
...
def create_menu_keyboard():
    ...
    moderation_text = "Модерация: вкл" if moderation_active else "Модерация: выкл"
...
async def send_last_messages(source_channel_id=None, limit=None):
    async def send_messages_to_destination(messages, destination_channel_id):
    ...
    if source_channel_id is not None:
        ...
        await send_messages_to_destination(messages, destination_channel_id)
    else:
        for src_id, dest_id in channel_mapping.items():
            chat = await client.get_entity(src_id)
            channel_messages = await client.get_messages(chat, limit=limit)
            await send_messages_to_destination(channel_messages, dest_id)
